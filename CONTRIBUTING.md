...
## XML Documentation and API Reference

### XML documentation standards

All public API members must carry XML documentation comments. The style used in this codebase:

- `<summary>` on every public type and member (and meaningful `internal` helpers)
- `<param>` for every parameter on non-trivial methods
- `<returns>` for every non-void return value
- `<exception cref="...">` where exceptions are thrown or documented
- `<remarks>` for non-obvious behaviour
- `<example>` for public entry-point classes where a usage snippet genuinely helps

When you add or change public API surface, add or update the XML doc comments in the same PR.

### Regenerating the API reference

The `docs/api/` folder contains the published API reference generated by [DocFX](https://dotnet.github.io/docfx/). After each NuGet release, regenerate it manually before committing the docs update:

```bash
scripts/regenerate-release-api-docs.sh <package-version>
```

The script restores tools, rebuilds the solution for DocFX, regenerates `docs/api/`, and writes `docs/api/release-notes-<package-version>.md`. Override `DOCFX_TARGET_FRAMEWORK` if you need a target other than the default `net10.0`.

To regenerate locally step by step:

```bash
# Ensure the .NET local tool is restored
dotnet tool restore
...
Commit any changed files under `docs/api/` when the published API reference changes. The intermediate `api/` YAML directory is gitignored and does not need to be committed.
