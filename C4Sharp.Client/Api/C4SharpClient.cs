...
namespace StacyClouds.C4Sharp.Api
{
    ...
    public class C4SharpClient
    {
        ...
        public C4SharpClient(string apiKey, string apiSecret) : this("https://api.structurizr.com", apiKey, apiSecret)
        {
        }
        ...
        /// <param name="apiKey">The API key of your workspace.</param>
        /// <param name="apiSecret">The API secret of your workspace.</param>
        public C4SharpClient(string apiUrl, string apiKey, string apiSecret)
        {
            Url = apiUrl;
            ApiKey = apiKey;
            ApiSecret = apiSecret;
            ...
            _version = typeof(C4SharpClient).GetTypeInfo().Assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>().InformationalVersion; 
        }
        ...
        private bool manageLockForWorkspace(long workspaceId, bool toBeLocked)
        {
            ...
            using (HttpClient httpClient = createHttpClient())
            {
                try
                {
                    ...
                    {
                        throw new C4SharpClientException(apiResponse.Message);
                    }
                }
                catch (Exception e)
                {
                    throw new C4SharpClientException("There was an error putting the workspace: " + e.Message, e);
                }
            }
        }
