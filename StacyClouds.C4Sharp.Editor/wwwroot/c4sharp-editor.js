window.c4sharpEditor = {
  initialize(host, component) {
    ...
    const pointerDown = e => { const node = e.target.closest('[data-c4-element-id]'), svg = e.target.closest('svg'); if (!node || !svg) return; drag = { id: node.dataset.c4ElementId, node, svg, center: elementCenter(node), connectors: Array.from(svg.querySelectorAll('[data-c4-relationship-id]')) }; e.preventDefault(); };
    const pointerMove = e => { if (!drag) return; const p = point(drag.svg, e.clientX, e.clientY); drag.node.setAttribute('transform', `translate(${p.x - drag.center.x},${p.y - drag.center.y})`); updateConnectorEndpoints(drag, p); };
...
function updateConnectorEndpoints(drag, point) { for (const connector of drag.connectors) { const points = connector.getAttribute('points').trim().split(/\s+/); if (connector.dataset.c4RelationshipSourceId === drag.id) points[0] = `${point.x},${point.y}`; if (connector.dataset.c4RelationshipDestinationId === drag.id) points[points.length - 1] = `${point.x},${point.y}`; connector.setAttribute('points', points.join(' ')); } }
